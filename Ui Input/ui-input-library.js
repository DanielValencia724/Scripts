    ...
    function updateStyle() {
        if (currentInputFieldScript) {
            if (domInPlace && currentInputFieldScript.entity.element.screenCorners) {
                ...
                var fontSize = Math.round(height * 0.5);
                inputDom.style.fontSize = fontSize + 'px';//Math.round(height * 0.5) + 'px';
                //inputDom.style.bottom = ((window.innerHeight/2) + fontSize + (currentInputFieldScript.entity.getPosition().y * 100)) + 'px';
                inputDom.style.bottom = null;
                inputDom.style.top = (cursorY - (height/2)) + 'px';  

                //console.log("--CurrentPosInputField: "+cursorY);
            } else {
                resetStyle();
            }
        }
